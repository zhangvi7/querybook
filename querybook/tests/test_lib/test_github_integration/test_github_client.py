...
from lib.github_integration.serializers import serialize_datadoc
...
def mock_github_link():
    return GitHubLink(
        ...
        repo_url="user/repo",
        branch="main",
        file_path="path/to/datadoc.json",
    )
...
def test_initialization(mock_flask_session, mock_github, mock_github_link, mock_repo):
    ...
    client = GitHubClient(mock_github_link)
...
def test_commit_datadoc_update(
    ...
    mock_github_instance = mock_github.return_value
    mock_github_instance.get_repo.return_value = mock_repo
    mock_contents = MagicMock(sha="fake_sha", path="path/to/datadoc.json")
    mock_repo.get_contents.return_value = mock_contents

    client = GitHubClient(mock_github_link)
    datadoc = DataDoc(id=1, title="Test Doc")
    serialized_content = serialize_datadoc(datadoc)
    client.commit_datadoc(datadoc, commit_message="Custom commit message")
    mock_repo.update_file.assert_called_once_with(
        path="path/to/datadoc.json",
...
def test_get_datadoc_versions(
    ...
    client = GitHubClient(mock_github_link)
    datadoc = DataDoc(id=1, title="Test Doc")
    versions = client.get_datadoc_versions(datadoc)
...
def test_get_datadoc_at_commit(
    ...
    mock_contents.decoded_content = b'{"id": 1, "title": "Test Doc", "meta": {}}'

    client = GitHubClient(mock_github_link)
    datadoc = client.get_datadoc_at_commit(1, "commit_sha")
    assert datadoc.id == 1
